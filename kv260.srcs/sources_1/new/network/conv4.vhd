...
architecture rtl of Conv4_Layer is
    ...
    subtype accumulate_t is signed(CONV4_ACCUM_WIDTH_C - 1 downto 0);
    ...
    begin
        if rising_edge(aclk) then
            ...
            if convolution_init = '1' then
                ...
                remaining_operations <= CONV4_CHAN_INPUT - 1;
                value <= (others => to_signed(0, CONV4_ACCUM_WIDTH_C));
                ...
                case operation_fsm is
                    ...
                    when WRITE_BACK =>
                        ...
                        for k in 0 to CONV4_CONCURRENT_KERNELS - 1 loop
                            ...
                            value(k) <= to_signed(0, CONV4_ACCUM_WIDTH_C);
                        end loop;
    ...
    begin
        if rising_edge(aclk) then
            ...
            if convolution_engine_stage_valid(1) = '1' then
                for k in 0 to CONV4_CONCURRENT_KERNELS - 1 loop
                    adder_tree_first(k)(0) <= resize(convolution_terms(k)(0) + convolution_terms(k)(1), CONV4_ACCUM_WIDTH_C);
                    adder_tree_first(k)(1) <= resize(convolution_terms(k)(2) + convolution_terms(k)(3), CONV4_ACCUM_WIDTH_C);
                    adder_tree_first(k)(2) <= resize(convolution_terms(k)(4) + convolution_terms(k)(5), CONV4_ACCUM_WIDTH_C);
                    adder_tree_first(k)(3) <= resize(convolution_terms(k)(6) + convolution_terms(k)(7), CONV4_ACCUM_WIDTH_C);
                    adder_tree_first(k)(4) <= resize(convolution_terms(k)(8), CONV4_ACCUM_WIDTH_C);
                end loop;
            end if;
