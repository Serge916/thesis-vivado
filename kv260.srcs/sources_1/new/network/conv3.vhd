...
architecture rtl of Conv3_Layer is
    ...
    type output_line_buffer_t is array (0 to CONV3_CONCURRENT_KERNELS - 1) of single_line_buffer_t;
        ...
        case central_column is
            when 0 =>
                for r in 0 to CONV3_KERNEL_SIZE - 1 loop
                    for c in 0 to (CONV3_KERNEL_SIZE/2) loop
                        if lines(r)(central_column + c) = '1' then
                            accumulated := accumulated + signed(kernel((c + (CONV3_KERNEL_SIZE/2)) + r * CONV3_KERNEL_SIZE));
                        end if;
                    end loop;
                end loop;
            when (CONV3_FRAME_WIDTH - 1) =>
                for r in 0 to CONV3_KERNEL_SIZE - 1 loop
                    for c in - (CONV3_KERNEL_SIZE/2) to 0 loop
                        if lines(r)(central_column + c) = '1' then
                            accumulated := accumulated + signed(kernel((c + (CONV3_KERNEL_SIZE/2)) + r * CONV3_KERNEL_SIZE));
                        end if;
                    end loop;
            ...
            when others =>
                for r in 0 to CONV3_KERNEL_SIZE - 1 loop
                    for c in - (CONV3_KERNEL_SIZE/2) to (CONV3_KERNEL_SIZE/2) loop
                        if lines(r)(central_column + c) = '1' then
                            accumulated := accumulated + signed(kernel((c + (CONV3_KERNEL_SIZE/2)) + r * CONV3_KERNEL_SIZE)); -- The kernel is from 0 to 8, that's why I add the limit
                        end if;
                    end loop;
                end loop;
        end case;
