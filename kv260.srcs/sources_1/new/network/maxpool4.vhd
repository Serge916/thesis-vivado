...
end entity Maxpool4_Layer;

architecture rtl of Maxpool4_Layer is
    ...
    constant FINISH_CONDITION : std_logic_vector(AXIS_TUSER_WIDTH_C - 1 downto 0) := std_logic_vector(to_unsigned(MAXPOOL4_OUTPUT_HEIGHT - 1, ROW_ID_WIDTH_C) & to_unsigned(CONV4_CHAN_OUTPUT - 1, CHANNEL_ID_WIDTH_C));

    --------------------------------------------------------------------------------
    -- Calculation signals
    --------------------------------------------------------------------------------
    signal output_line : axis_word_t;
    subtype channel_line_buffer_t is std_logic_vector(MAXPOOL4_TDATA_WIDTH - 1 downto 0);
    type line_buffer_t is array (0 to CONV4_CONCURRENT_KERNELS - 1) of channel_line_buffer_t;
...
begin
    ...
    lines_in : process (aclk)
        ...
        variable channel_id : natural range 0 to CONV4_CHAN_OUTPUT - 1 := 0;
    begin
        ...
        if rising_edge(aclk) then
            ...
            if axi_in_ready = '1' and s_axis_tvalid = '1' then
                ...
                if row_id(0) = '0' then
                    ...
                    line_buffer(channel_id mod CONV4_CONCURRENT_KERNELS) <= s_axis_tdata; -- First index depends on how big the batch is. Second index is 0 if even, 1 if uneven row
                else
                    -- Compute
                    for c in 0 to MAXPOOL4_OUTPUT_WIDTH - 1 loop -- The 2 is for the stride
                        output_line.tdata(c) <= line_buffer(channel_id mod CONV4_CONCURRENT_KERNELS)(2 * c) or s_axis_tdata(2 * c) or line_buffer(channel_id mod CONV4_CONCURRENT_KERNELS)(2 * c + 1) or s_axis_tdata(2 * c + 1);
                    end loop;
